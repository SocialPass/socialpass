...
class ModelObjectTracker:
    ...
    def __init__(self, request, uuid: typing.Union[str, UUID]):
        self.request = request
        self.data = self.data_backend(request)
        self.uuid = self.data.safe_cast_to_uuid(uuid)

        try:
            obj_data = self.data[self.uuid]
        except KeyError:
            raise SuspiciousOperation(
                "Tracking UUID was not generated by the ModelObjectTracker"
            )
        else:
    ...
    def __deserialize_object(self, obj_data):
        Model = apps.get_model(obj_data["model_app"], obj_data["model_name"])
        try:
