...
# Fonts (and related)
BASE_FONT_TYPE = finders.find("redesign/AvertaBold.otf")
...
EVENT_GATE_LIMIT_WIDTH = int(DEFAULT_WIDTH * (1/3))
EVENT_DATE_WIDTH = int(DEFAULT_WIDTH * (2/3))
...
def fit_text(
    ...
    for t, w, h in pieces:
        ...
        new_width = draw.textlength(t, font=font)
        y += h
        new_height = 10 # TODO
...
class TicketPartGenerator:
    ...
    def generate_banner(width, height, bg_color, color, font, text=None):
        ...
        if text:
            draw = ImageDraw.Draw(img)
            width_text = draw.textlength(text, font)
            height_text = 10 # TODO

            offset_x = font.getlength(text)
            offset_y = 10 # TODO
            width_text += offset_x
    ...
    def generate_scene_image(source, max_height):
        ...
        scene_img = scene_img.resize(
            (wsize, max_height), Image.LANCZOS
        )
...
def generate_ticket(
    event_data, embed, theme=None, top_banner_text='NFTY Labs Ticket',
    ...
    ticket_parts['event_gate_limit'] = concat_v(
        TicketPartGenerator.generate_text_section(
            ...
            font=TITLE_FONT,
            x_start_offset=DEFAULT_MARGIN,
            x_end_offset=int(DEFAULT_MARGIN / 2),
            v_spacing=0
        ),
        TicketPartGenerator.generate_text_section(
            width=EVENT_GATE_LIMIT_WIDTH,
    ...
    # Scene image
    if scene_img_source:
        ticket_parts['scene_img'] = TicketPartGenerator.generate_scene_image(
