...
const devURL = '192.168.0.10:8000/api'
...
export function fetchEvent(publicId: string) {
  return fetchApi(api.get(`scanner/v1/${publicId}/event/`))
}

export function fetchScanTicket(publicId: string, qrCode: string) {
  return fetchApi(api.post(`scanner/v1/${publicId}/claim-ticket/`, { embed_code: qrCode }))
}
