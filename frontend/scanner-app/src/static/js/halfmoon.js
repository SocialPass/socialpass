  ...
  Object.defineProperty(Object, 'assign', {
    ...
    value: function (e) {
      ...
      for (var r = Object(e), t = 1; t < arguments.length; t++) {
        ...
        if (null != n) {
          n = Object(n)
          for (var c = Object.keys(Object(n)), o = 0, a = c.length; o < a; o++) {
            var b = c[o],
              i = Object.getOwnPropertyDescriptor(n, b)
            void 0 !== i && i.enumerable && (r[b] = n[b])
          }
        }
      }
      return r
...
var halfmoon = {
  ...
  scrollXCenter: function (container, target) {
    var scrollX = target.offsetLeft + target.clientWidth / 2 - container.clientWidth / 2
  ...
  toggleDarkMode: function () {
    if (document.documentElement.classList.contains('dark-mode')) {
      ...
      halfmoon.createCookie('halfmoon_preferredColorScheme', 'light-mode', 365)
    } else {
      document.documentElement.classList.add('dark-mode')
      halfmoon.createCookie('halfmoon_preferredColorScheme', 'dark-mode', 365)
    }
  },
  ...
  autosetPreferredColorScheme: function () {
    ...
    var autosetPreferences =
      document.documentElement.getAttribute('data-hm-autoset-preferences') || ''
  ...
  toggleReadableMode: function () {
    if (document.documentElement.classList.contains('readable-mode')) {
      ...
      halfmoon.createCookie('halfmoon_preferredFontSize', 'compact-mode', 365)
    } else {
      document.documentElement.classList.add('readable-mode')
      halfmoon.createCookie('halfmoon_preferredFontSize', 'readable-mode', 365)
    }
  },
  ...
  queryNavbars: function () {
    ...
    var navbar = document.querySelector('.page-wrapper > .navbar:not(.navbar-fixed-bottom)')
    if (navbar) {
      halfmoon.navbars.top = navbar

      // Calculate the height (in px)
      var heightTop = getComputedStyle(navbar)['height']
      heightTop = Number(heightTop.substring(0, heightTop.length - 2))
      halfmoon.navbars.heightTop = heightTop
    }

    // Navbar fixed bottom
    var navbarFixedBottom = document.querySelector('.page-wrapper > .navbar-fixed-bottom')
    if (navbarFixedBottom) {
      halfmoon.navbars.bottom = navbarFixedBottom

      // Calculate the height (in px)
      var heightBottom = getComputedStyle(navbarFixedBottom)['height']
      heightBottom = Number(heightBottom.substring(0, heightBottom.length - 2))
  ...
  supportsNavbarFixedBottom: function () {
    if (halfmoon.pageWrapper) {
      if (halfmoon.pageWrapper.classList.contains('with-navbar-fixed-bottom')) {
        if (!halfmoon.navbars.queried) {
          halfmoon.queryNavbars()
        }
      ...
      if (halfmoon.contentWrapper.contains(target)) {
        if (halfmoon.supportsNavbar()) {
          ...
          if (threshold < halfmoon.navbars.heightTop) {
            window.scrollTo(0, window.scrollY - halfmoon.navbars.heightTop)
          }
        }
      }
    }
  ...
  querySidebar: function () {
    ...
    if (sidebar) {
      ...
      if (halfmoon.pageWrapper.hasAttribute('data-hm-sidebar-type')) {
        halfmoon.sidebar.type = halfmoon.pageWrapper.getAttribute('data-hm-sidebar-type')
      }
      ...
      var transitionDuration = getComputedStyle(sidebar)['transition-duration']
      transitionDuration = transitionDuration.replace(/\s/g, '')
      transitionDuration = transitionDuration.split(',')[0]
      transitionDuration = Number(transitionDuration.substring(0, transitionDuration.length - 1))
      ...
      var sidebarOverlay = document.querySelector('.page-wrapper > .sidebar-overlay')
      if (!sidebarOverlay) {
  ...
  closeSidebar: function () {
    if (halfmoon.supportsSidebar()) {
      if (window.innerWidth > 768) {
        halfmoon.pageWrapper.setAttribute('data-hm-sidebar-hidden', 'true')
    ...
    if (halfmoon.supportsSidebar()) {
      if (window.innerWidth > 768) {
        if (halfmoon.sidebar.type.includes('overlayed-all')) {
          halfmoon.pageWrapper.setAttribute('data-hm-sidebar-hidden', 'false')
  ...
  toggleSidebar: function () {
    if (halfmoon.supportsSidebar()) {
      ...
      if (halfmoon.pageWrapper.hasAttribute('data-hm-sidebar-hidden')) {
        var sidebarHidden = halfmoon.pageWrapper.getAttribute('data-hm-sidebar-hidden')
  ...
  getTransitionDurationForClose: function () {
    ...
    if (halfmoon.currentDialog.obj) {
      if (halfmoon.currentDialog.obj.classList.contains('offcanvas')) {
        ...
        transitionDuration = getComputedStyle(halfmoon.currentDialog.obj)['transition-duration']
  ...
  animateDialogShake: function () {
    if (halfmoon.currentDialog.obj) {
      ...
      if (dialog.classList.contains('modal')) {
        ...
        animationDuration = getComputedStyle(dialog.querySelector('.modal-dialog'))[
          'animation-duration'
        ]
      } else if (dialog.classList.contains('offcanvas')) {
        // For offcanvas, the animation duration is on itself
        animationDuration = getComputedStyle(dialog)['animation-duration']
      }

      // Convert the duration to milliseconds (default is in seconds)
      animationDuration = Number(animationDuration.substring(0, animationDuration.length - 1))
  ...
  closeAlert: function (alert) {
    if (alert) {
      ...
      var transitionDuration = getComputedStyle(alert)['transition-duration']
      transitionDuration = transitionDuration.replace(/\s/g, '')
      transitionDuration = transitionDuration.split(',')[0]
      transitionDuration = Number(transitionDuration.substring(0, transitionDuration.length - 1))
      transitionDuration = transitionDuration * 1000
  ...
  queryStickyAlerts: function () {
    if (!halfmoon.stickyAlerts) {
      var stickyAlerts = document.querySelector('.page-wrapper > .sticky-alerts')
    ...
    if (halfmoon.pageWrapper) {
      ...
      if (toggle.hasAttribute('data-hm-target')) {
        ...
        toast = document.getElementById(
          halfmoon.getIdFromTargetAttr(toggle.getAttribute('data-hm-target')),
        )
      } else {
        ...
        var toastOpts = {
          ...
          dismissible: toggle.getAttribute('data-hm-dismissible') || true,
        ...
        halfmoon.stickyAlerts.insertBefore(toast, halfmoon.stickyAlerts.childNodes[0])
      }
  ...
  closeDropdown: function (focusToToggle) {
    if (halfmoon.currentDropdown.menu) {
      ...
      if (halfmoon.currentDropdown.toggle) {
        ...
        halfmoon.currentDropdown.toggle.setAttribute('aria-expanded', 'false')
      }
  ...
  activateTab: function (tabPaneId, toggle) {
    ...
    if (tabPane) {
      ...
      var relatedToggles = toggle.closest('.tabs').querySelectorAll("[data-hm-toggle='tab']")
      ...
      var relatedTabPanes = tabPane.closest('.tab-content').querySelectorAll('.tab-pane')
  ...
  scrollTabsToActiveToggle: function () {
    var activeTabToggles = document.querySelectorAll("[data-hm-toggle='tab'].active")
  ...
  initIndeterminateInputs: function () {
    var indeterminateInputs = document.querySelectorAll('[data-hm-indeterminate]')
  ...
  initFileInputs: function () {
    ...
    for (var i = 0; i < fileInputs.length; i++) {
      ...
      fileInput.addEventListener('change', function (event) {
        filename = event.target.parentNode.querySelector('.form-file-filename')
  ...
  bindInputValue: function (input) {
    if (input.getAttribute('data-hm-target')) {
      ...
      for (var i = 0; i < targetIds.length; i++) {
        target = document.getElementById(halfmoon.getIdFromTargetAttr(targetIds[i]))
        if (target) {
  ...
  bindInputs: function () {
    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (!halfmoon.isInternetExplorer()) {
        inputs[i].addEventListener('input', halfmoon.callBindInputValueForAttachment)
      } else {
        ...
        if (
          ...
          inputs[i].addEventListener('change', halfmoon.callBindInputValueForAttachment)
        }
      }
    }
  },
    ...
    if (event.keyCode === 9) {
      if (halfmoon.supportsSidebar()) {
        ...
        // Or if the sidebar type is "overlayed-all"
          ...
          if (halfmoon.sidebar.type.includes('overlayed-all') && sidebarHidden === 'false') {
            if (!halfmoon.sidebar.obj.contains(event.target)) {
              halfmoon.closeSidebar()
            }
          }
      ...
      if (halfmoon.contentWrapper.contains(event.target)) {
        ...
        if (halfmoon.supportsNavbarFixedBottom()) {
          ...
          if (threshold < halfmoon.navbars.heightBottom) {
            ...
            window.scrollTo(0, window.scrollY + halfmoon.navbars.heightBottom)
          }
        }
      }
    }
  ...
  onClick: function (event) {
    ...
    if (!('scroll-margin-top' in document.documentElement.style)) {
      if (target.matches('a[href]')) {
        ...
        if (target.getAttribute('href') === location.hash) {
          var hashTarget = document.getElementById(target.getAttribute('href').substring(1))
          if (hashTarget) {
            event.preventDefault()
            halfmoon.adjustContentScrollMargin(hashTarget)
          }
    ...
    if (
      ...
      halfmoon.openDialog(
        halfmoon.getIdFromTargetAttr(target.getAttribute('data-hm-target')),
        target,
      )
      event.preventDefault()
    }
    ...
    if (target.matches('.modal-dialog') || target.matches('.dialog-overlay')) {
      var dialog = halfmoon.currentDialog.obj
    ...
    if (
      ...
      if (!target.classList.contains('menu-open')) {
        ...
        var menu = target.parentElement.querySelector('.dropdown-menu')
    ...
    if (target.matches("[data-hm-toggle='tab']") || target.matches("[data-hm-toggle='tab'] *")) {
    ...
    if (
      ...
      halfmoon.inputStepUp(halfmoon.getIdFromTargetAttr(target.getAttribute('data-hm-target')))
      event.preventDefault()
    }
    ...
    if (
      ...
      halfmoon.inputStepDown(halfmoon.getIdFromTargetAttr(target.getAttribute('data-hm-target')))
      event.preventDefault()
    }
